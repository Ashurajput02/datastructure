...
void multiplication()
{
    ...
    while (jig != NULL)
    {
        while (sim != NULL)
        {
           ...
           struct node *temp4 = insert4(a, b);
           struct node *temp5=head4;
           while(temp5!=NULL){
            if(temp5==temp4)
            break;
            
            if(temp5->data2==temp4->data2){
                temp5->data1+=temp4->data1;
...
{
    struct ashu *temp = (struct node *)malloc(sizeof(struct ashu));
    if (temp == NULL)
    {
        printf("Memory overflow!\n");
        exit(EXIT_FAILURE);
